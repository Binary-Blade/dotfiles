if initialize_session "Config"; then
  ...
  run_cmd "cd ~/.config/tmux && nvim ." || echo "Failed to run 'cd ~/.config/tmux && nvim .'"

  new_window "C-Tmuxifier" || echo "Failed to create C-Tmuxifier window"
  run_cmd "cd ~/.config/tmux/.tmuxifier/layouts && nvim ." || echo "Failed to run 'cd ~/.config/tmux/.tmuxifier/layouts && nvim .'"

  new_window "C-Envman" || echo "Failed to create C-Envman window"
  run_cmd "cd ~/.config/envman && nvim ." || echo "Failed to run 'cd ~/.config/envman && nvim .'"
